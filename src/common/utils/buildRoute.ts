...
export const buildRoutePath = (parsedFile: ParsedPath): string => {
	...
	const normalizedDir =
		parsedFile.dir === parsedFile.root ? "/" : parsedFile.dir.startsWith("/") ? parsedFile.dir : `/${parsedFile.dir}`;
...
export const calculatePriority = (url: string) => {
	...
	const catchall = (url.match(/\/\*/g)?.length ?? 0) > 0 ? Number.POSITIVE_INFINITY : 0;
